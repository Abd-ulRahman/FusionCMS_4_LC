...
class Arcemu implements Emulator
{
	...
	private $queries = array(
		"get_character" => "SELECT guid guid,acct account,name name,race race,class class,gender gender,level level,current_hp current_hp,current_power current_power,zoneId zone,online online,gold money,killsLifeTime totalKills,arenaPoints arenaPoints,honorPoints totalHonorPoints,positionX position_x,positionY position_y,positionZ position_z,orientation orientation,mapId map FROM characters WHERE guid=?",
		...
		"get_account_id" => "SELECT acct id, login username, encrypted_password password, password password2, email email, lastlogin joindate, lastip last_ip, lastlogin last_login, flags expansion FROM accounts WHERE acct = ?",
		"get_account" => "SELECT acct id, login username, encrypted_password password, password password2, email email, lastlogin joindate, lastip last_ip, lastlogin last_login, flags expansion FROM accounts WHERE login = ?",
		...
		"get_inventory_item" => "SELECT slot slot, guid item, entry itemEntry FROM playeritems WHERE entry < 56807 AND slot >= 0 AND slot <= 18 AND containerslot = -1 AND ownerguid = ? ORDER BY guid ASC",
	...
	public function encrypt($username, $password)
	{
		...
		$encrypted_password = sha1(strtoupper($username).':'.strtoupper($password));
	 ...
	 * Password encryption
	 */
	public function encrypt2($username, $password2)
	{
		return $password2;
	}
