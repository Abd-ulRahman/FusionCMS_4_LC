	...
	*/
	private function getArcemuProfessions()
	{
		...
		if ($this->storedData)
		{
			$this->connect();
			//Define the professions (skill ids) we need
			$professions = "164,165,171,182,186,197,202,333,393,755,773,129,185,356,794";

			$query = $this->connection->query("SELECT `SkillID`, `CurrentValue`, `MaximumValue` FROM `playerskills` WHERE `guid` = ? AND `SkillID` IN(".$professions.");", array($this->id));
			...
			if ($result)
			{
				...
				foreach ($result as $key => $row)
				{
					if ($info = $this->getProfessionInfo((int)$row['SkillID']))
					{
						$result[$key] = array(
							'skill' 	=> $row['SkillID'],
